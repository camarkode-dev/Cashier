...
exports.thermalPrinter = exports.ThermalPrinter = void 0;
...
class ThermalPrinter {
    ...
    async connectUSB() {
        try {
            ...
            this.endpoint = iface.alternate.endpoints.find((e) => e.direction === 'out') || null;
            return true;
        }
        catch (err) {
    ...
    buildReceipt(data) {
        ...
        if (data.storeAddress)
            pos.align('center').line(data.storeAddress);
        if (data.storePhone)
            pos.align('center').line('هاتف: ' + data.storePhone);
        pos.divider('=', width);
        // Invoice info
        ...
        if (data.branchName)
            pos.line('الفرع: ' + data.branchName);
        if (data.customerName)
            pos.line('العميل: ' + data.customerName);
        pos.divider('-', width);
        // Items
        pos.align('right');
        ...
        if (data.discountAmount > 0)
            pos.line(`الخصم: -${data.discountAmount.toFixed(2)} ${cur}`);
        if (data.taxAmount > 0)
            pos.line(`الضريبة: ${data.taxAmount.toFixed(2)} ${cur}`);
        pos.bold(true).size(1, 2);
        pos.line(`الإجمالي: ${data.total.toFixed(2)} ${cur}`);
        pos.bold(false).size(1, 1);
        pos.line(`المدفوع: ${data.paidAmount.toFixed(2)} ${cur}`);
        if (data.changeAmount > 0)
            pos.line(`المتبقي: ${data.changeAmount.toFixed(2)} ${cur}`);
        // Loyalty
        if (data.loyaltyPoints)
            pos.line(`نقاط الولاء: ${data.loyaltyPoints} نقطة`);
        pos.divider('=', width);
        pos.align('center').line('شكراً لتسوقكم معنا');
        if (data.footer)
            pos.line(data.footer);
        pos.feed(4).cut();
        return pos.getBytes();
    }
    async printViaUSB(data) {
        if (!this.device || !this.endpoint)
            return;
    ...
    async sendToNetworkPrinter(ip, port = 9100, data) {
...
exports.thermalPrinter = new ThermalPrinter();
