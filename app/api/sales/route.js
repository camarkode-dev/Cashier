...
async function POST(req) {
    ...
    if (!dbUser)
        return (0, api_utils_1.unauthorized)();
    try {
        const body = validations_1.saleSchema.parse(await req.json());
        // Check for duplicate offline sale
        if (body.offlineId) {
            const existing = await prisma_1.prisma.sale.findUnique({ where: { offlineId: body.offlineId } });
            if (existing)
                return (0, api_utils_1.ok)({ ...existing, duplicate: true });
        }
        const invoiceNumber = await (0, api_utils_1.generateInvoiceNumber)(body.branchId);
        const sale = await prisma_1.prisma.$transaction(async (tx) => {
            let subtotal = 0;
            let totalTax = 0;
            let totalProfit = 0;
            // Validate & compute each item
            const enrichedItems = await Promise.all(body.items.map(async (item) => {
          ...
          `;
                if (!inv || inv.quantity < item.quantity) {
                    ...
                    throw new Error(`مخزون غير كافٍ للمنتج ${productLabel}. المتاح: ${inv?.quantity ?? 0}`);
                }
        ...
        await (0, api_utils_1.audit)(dbUser.id, 'CREATE', 'sale', sale.id, { total: sale.total, invoiceNumber });
        return (0, api_utils_1.created)(sale);
    }
    catch (e) {
